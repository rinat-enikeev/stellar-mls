...
object Bip39 {
    ...
    fun mnemonicFromEntropy(entropy: ByteArray): String {
        ...
        for (i in entropy.indices) {
            for (j in 7 downTo 0) {
                bits[i * 8 + (7 - j)] = (entropy[i].toInt() shr j) and 1 == 1
            }
        }
        // Checksum bits
        for (i in 0 until checksumBits) {
            val shift = 7 - i
            bits[entropy.size * 8 + i] = (hash[0].toInt() shr shift) and 1 == 1
        }

        val wordCount = bits.size / 11
        return (0 until wordCount).joinToString(" ") { i ->
            var index = 0
            for (j in 0 until 11) {
                if (bits[i * 11 + j]) {
    ...
    /** Convert a BIP39 mnemonic back to the original entropy bytes. Returns null if invalid. */
    fun entropyFromMnemonic(mnemonic: String): ByteArray? {
        val words = mnemonic.lowercase().trim().split("\\s+".toRegex())
        ...
        for ((w, word) in words.withIndex()) {
            ...
            for (j in 10 downTo 0) {
                bits[w * 11 + (10 - j)] = (index shr j) and 1 == 1
            }
        }
        ...
        val hash = MessageDigest.getInstance("SHA-256").digest(entropy)
        for (i in 0 until checksumBits) {
            val expected = (hash[0].toInt() shr (7 - i)) and 1 == 1
    ...
    fun deriveNostrKey(seed: ByteArray): ByteArray =
        GroupCrypto.hkdf(
            ikm = seed,
            salt = "chat.onym.bip39".toByteArray(),
            info = "nostr-secp256k1-v1".toByteArray(),
            length = 32
        )

    /** Derive BLS12-381 secret key (32 bytes) from BIP39 seed via HKDF-SHA256. */
    fun deriveBlsKey(seed: ByteArray): ByteArray =
        GroupCrypto.hkdf(
            ikm = seed,
            salt = "chat.onym.bip39".toByteArray(),
            info = "bls12-381-v1".toByteArray(),
            length = 32
        )
}
