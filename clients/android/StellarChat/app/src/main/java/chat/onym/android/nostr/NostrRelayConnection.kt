...
class NostrRelayConnection(
    ...
    private fun handleMessage(text: String) {
        ...
        if (text.length > MAX_MESSAGE_SIZE) {
            chat.onym.android.SecurityLog.relayEvent("oversized message rejected (${text.length} bytes)", url)
            return
        }
        try {
            ...
            when (array.getString(0)) {
                ...
                "OK" -> {
                    if (array.length() >= 3) {
                        ...
                        if (chat.onym.android.BuildConfig.DEBUG) android.util.Log.d("Relay", "OK from $url eventId=${eventId.take(12)} accepted=$accepted reason=$reason")
            ...
            chat.onym.android.SecurityLog.relayEvent("malformed message: ${e.message}", url)
        }
    }
    ...
    private fun parseEvent(json: JSONObject): NostrEvent? {
        return try {
            ...
            if (!event.verifyEventID()) {
                chat.onym.android.SecurityLog.relayEvent("invalid event ID", url)
                return null
            }
            event
