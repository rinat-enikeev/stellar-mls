...
`archive_entry` (`lib.rs:853-872`) appends and prunes from the front. Test pin: `test_archive_entry_appends_and_prunes`. Entries pruned from contract storage are still present in contract events (`CommitmentUpdated`), per design §A.
