...
| Group inactive (i.e., `current.active == false` post-deactivation) | `Ok(true)` for a valid membership proof against the final pre-deactivation `(commitment, epoch)`; `Ok(false)` for an invalid proof. The read-only verifier intentionally does NOT check `state.active` — post-deactivation attestations against the frozen final state remain verifiable forever. | `test_verify_membership_rejects_inactive_group` (the test name is historical; what it actually pins is "verifier reaches `Ok(false)` on an inactive group with a mock proof", confirming the absence of an `if !state.active` short-circuit. Phase A's real proofs would yield `Ok(true)` if valid against the final state.) |
...
9. **`Error::InvalidEpoch` reused for u64-overflow** (per PR #149 review releaseng chunk 1 #3). The same error code is used both for "wire `epoch_old` doesn't match `current.epoch`" (semantic state-chain mismatch — already covered by `PublicInputsMismatch=10`) and for `current.epoch.checked_add(1)` overflow in `update_commitment`. The two paths semantically differ: the first is "wire is stale", the second is "u64 saturated counter, should never happen for ~10^19 epochs." Cosmetic, realistically unreachable. Could split into `EpochOverflow=11` + `EpochMismatch=10` in a future revision; not blocking.

10. **`update_commitment` allows no-op heartbeats** (per PR #149 review releaseng chunk 1 #4). A successful update with `c_new == c_old` (and identical `occupancy_commitment_new == occupancy_commitment_old`) is technically possible if the prover constructs such a proof — it would consume one history slot, advance the epoch, and burn a nullifier without changing the bitmap state. This is harmless griefing the admin quorum can do to itself; semantically it represents a "key rotation" or no-op heartbeat. Intentional and documented; the contract has no special path to reject c_new==c_old.
