...
| SI-4 | Code structure. `update_commitment` writes `new_entry.admin_pubkey_commitment = current.admin_pubkey_commitment`. The wire payload `UpdatePublicInputs` does not carry `admin_pubkey_commitment` — there is no path for caller-supplied data to reach `new_entry.admin_pubkey_commitment`. No `update_admin` entrypoint exists in v0. |
