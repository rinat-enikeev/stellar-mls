...
2. **Stable slot ↔ identity invariant.** Clients persist `(memberPubkey, slotIndex)` pairs. If a removed member's slot were reassigned to a new member, every client's local mapping would have to be invalidated/migrated at exactly the right epoch, requiring synchronous purge-then-reassign coordination. Permanent tombstones let the slot map grow monotonically *within the lifetime of a group*: clients only need to learn new mappings, never forget old ones. **Caveat**: §10 Q1 originally left open whether a `groupSecret`-reuse reset (deactivate + recreate) would constitute a fresh group. Postmortem #153 dropped `deactivate_group` entirely — there is no on-chain reset path. The remaining question is whether `groupSecret`-reuse via off-chain re-creation (clients abandon the old group_id and create a new one with a fresh `group_id`) is treated as a clean slate; recommendation stands as "yes, a fresh group_id resets the slot map." Old groups can no longer be deactivated on-chain and simply age out via TTL.
...
3. **Slot reuse policy after group reset.** If a group is abandoned (no on-chain `deactivate_group` since postmortem #153 removed that entrypoint) and a new one created with the same `groupSecret` but a fresh `group_id`, do slot indices reset? Yes — a fresh `group_id` is a new contract entry, with a fresh slot map. §4.1.2's "monotonic, never forget" property holds *per group_id*; off-chain re-creation is the legitimate path to a clean slate. Confirmed during D5 client-state work.
