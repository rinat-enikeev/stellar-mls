...
**Two circuits per tier.** The SEP defines two Groth16 circuits per tier: a *membership* circuit (`R_Membership`, 2 public inputs) used for `create_group`, `verify_membership`, and `deactivate_group`; and an *update* circuit (`R_Update`, 3 public inputs) used for `update_commitment`. The update circuit exists because state transitions must bind the *new* commitment as a public input of the proof — not just the old one — to prevent an observer of a valid proof from substituting a different `new_commitment` in the transaction envelope (see `docs/vuln-unbound-new-commitment.md`). Each tier therefore ships two verification keys. The proof size, pairing count, and constant-verification-cost property are identical across both circuits.
