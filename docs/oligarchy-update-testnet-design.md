...
**Version:** 0.1.5 — withdraws the §3.5 "which tree changed in any given update" hiding claim and the §4.7.7 single-leaf-delta + `target_tree` dispatcher mechanism that backed it. Issue #217 noted the v2 oligarchy circuit ships without §4.7.3 constraints #7–8 (single-leaf delta on the target tree, non-target-tree no-change), making the §3.5 last-row claim unenforced in the deployed VK. Re-litigation: admin quorum is the design's authorization mechanism — admins are intended to be able to re-shape governance (multi-leaf rotations across either tree, or both atomically). The "uniform proof byte length, hidden tree dispatcher" privacy axis is a layered nice-to-have, not load-bearing for the v2 release; the privacy gain (~one bit of "which tree changed" entropy hidden per update) is not worth the ~570 R1CS, ~+25% circuit complexity, and the new `gen-oligarchy-update-proof` binary the dispatcher would require. v0.1.5 removes the unimplemented spec rows and explicitly accepts "tree identity per update" as a residual (alongside admin tree depth and the existence-of-admin-set residuals already enumerated). The deployed circuit's behaviour stays as-is: K-of-N admin quorum + count-only delta + admin-tree membership + commitment-chain reconciliation. Side effects: §3.3 inheritance from Democracy §3.3 narrowed to the member tree only (admin tree no longer carries the single-leaf invariant); §4.7.3 R1CS estimate drops by ~570 (tier-1 ~10700, tier-0 ~8330, tier-2 ~24000); §10.4 "cross-tree atomic updates" is reframed from "out of scope" to "supported via multi-leaf updates on both trees in a single proof, subject to the quorum"; §11 follow-up entry for tree-level delta is closed with a wontfix link to issue #217. Sub-issue: claim 1 of issue #217 (threshold range mismatch — contract accepts `[1,100]`, circuit only handles `[0,3]`) is **not** addressed by this version; tracked separately.
...
### 3.3 Single-leaf delta on the member tree (narrowed in v0.1.5)

Same constraint as [Democracy §3.3](democracy-update-testnet-design.md#33-single-leaf-delta-requires-a-stable-slot-index-convention), applied to the **member tree only**. §4.1's slot-index convention extends to the member tree to keep the count-only delta gadget aligned with Democracy's; the admin tree no longer carries this invariant in v0.1.5.

**v0.1.5 narrowing.** Earlier drafts (v0.1.0–v0.1.4) inherited the single-leaf invariant for both trees as a precondition for §3.5's "which tree changed" hiding claim. v0.1.5 withdraws that claim (see §3.5 below + version stanza), so the admin tree no longer needs the same constraint. Admin updates are now multi-leaf — a single proof can rotate any subset of admin slots, subject to the K-of-N quorum check on the *prior* admin set. Cross-tree atomic updates (member + admin in one proof) are likewise allowed.
...
| Which tree changed in any given update (member vs admin) | (n/a) | (n/a — single tree) | ~~✓ (private witness, see §4.7.7)~~ **WITHDRAWN in v0.1.5** — accepted as a residual (see below). The §4.7.7 dispatcher mechanism is no longer part of the spec. |

**v0.1.5 — withdrawal of the "which tree changed" hiding claim.** The privacy mechanism (single-leaf delta on the target tree + private `target_tree` witness + non-target-tree no-change) was specified across §3.3, §4.7.3 (constraints #7–8), and §4.7.7. Issue #217 surfaced that the deployed PLONK circuit ships without those constraints; re-evaluating the trade, the privacy gain (~1 bit of "which tree changed" entropy hidden per update — and only against an on-chain-only observer; out-of-band access already defeats it) is not worth the ~570 R1CS, the ~25% circuit-size growth, the dedicated `gen-oligarchy-update-proof` binary, the dispatcher gadget, and the multi-leaf-update friction the constraint imposes on legitimate admin operations. The deployed circuit's actual semantics (K-of-N admin quorum + count-only member delta + admin-tree signer membership + commitment chain reconciliation) match Oligarchy's authorization model: admins are entitled to re-shape both trees, multi-leaf, in a single proof. The remaining §3.5 rows above are unchanged.
...
**One per-update residual (added in v0.1.5).** "Which tree changed in any given update" — a chain observer can distinguish member-tree updates from admin-tree updates by the occupancy-commitment fingerprint (admin tree has 32 slots; member tree has 32 / 256 / 2048 slots depending on tier). Earlier drafts treated this as a §3.5 MUST-hide; v0.1.5 accepts it as a residual to keep the circuit complexity bounded and to allow multi-leaf admin operations under quorum. The salt-protected combined occupancy commitment (v0.1.4) still defeats brute-force-from-public-priors recovery of bitmap evolution; the residual is specifically about the per-update "did this proof touch member side or admin side."
...
7. ~~**Single-leaf delta on the target tree** (§4.7.7's `target_tree` witness picks which tree)~~. **WITHDRAWN in v0.1.5** — see §3.5 narrative. The deployed circuit allows multi-leaf updates on either or both trees; the prior `target_tree` witness is removed from the spec. Member-side count delta is preserved (count-only, parallel to Democracy) but the new member-tree root is a free witness bound only by the count.
8. ~~**Non-target-tree no-change**~~. **WITHDRAWN in v0.1.5** — admin and member trees can both change in a single update, subject only to constraint #4's quorum check on the prior admin set.
...
#### 4.7.7 Hiding which tree changed (~~the new privacy axis for Oligarchy~~ — **WITHDRAWN in v0.1.5**)

> **WITHDRAWN.** v0.1.5 removes the §3.5 "which tree changed" hiding claim and the dispatcher mechanism this section described. The text below is preserved verbatim for historical context (PR review trail, issue #217); none of it reflects the deployed v2 circuit. The deployed circuit allows multi-leaf updates on either or both trees and does not carry a `target_tree` private witness. See §3.5 (last row + residual entry) and the v0.1.5 stanza for the trade-off rationale.
...
### 4.10 Cross-tree atomic updates (~~NOT in v2~~ — **supported as of v0.1.5**)

v0.1.5 withdraws the §4.7.7 single-leaf-delta dispatcher. A single update that changes both trees simultaneously (e.g., "promote Alice to admin AND add Bob as member" in one epoch) is now **in scope** — the deployed circuit imposes no tree-identity restriction beyond the K-of-N admin quorum check on the prior admin set. The earlier "constraint count +50%" overhead the dispatcher imposed on cross-tree updates is not paid.
...
- ~~**Cross-tree atomic updates.**~~ **Closed by v0.1.5** — the §4.7.7 dispatcher withdrawal makes cross-tree atomic updates a free byproduct of the deployed circuit. No follow-up needed.
