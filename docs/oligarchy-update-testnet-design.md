...
Occupancy commitments (v0.1.4): a single **salted, combined** commitment is exposed on the wire and in storage; per-tree intermediates and the per-epoch salt are circuit-internal:
...
occupancy_commitment          = Poseidon(DOMAIN_COMBINED_OCCUPANCY,
                                         member_occupancy_intermediate,
                                         admin_occupancy_intermediate,
                                         salt_occ)
```

`salt_occ` is a fresh per-epoch private-witness scalar (32-byte canonical Fr, sampled by the prover at proof generation). It is NOT on the wire and NOT in contract storage. Distributed off-chain via `SEPSaltResponse.occupancySalt` (§4.4) so subsequent updates' provers can recompute the prior epoch's commitment to chain forward. Same off-chain-distribution shape Democracy already uses for `c_new`'s salt.

The admin bitmap (32 bits) packs into a single `Fr` scalar (252-bit capacity), so admin's `fold` is trivial — one scalar in, one Poseidon call out. Member side scales with tier as in Democracy (1 / 2 / 9 scalars). The combine adds one Poseidon hash per side (old / new) — ~600 R1CS in total. v0.1.4's salt is absorbed as a separate Poseidon hash chained after the combine (`occupancy_commitment = Poseidon(Poseidon(DOMAIN_COMBINED_OCCUPANCY, member_occ, admin_occ), salt_occ)`); this is the implementation-agnostic worst case (avoids depending on a 3-input Poseidon arity being available in the prover library). One full hash per side, ~300 R1CS each, ~600 R1CS over both sides. The conceptual formula in the snippet above is implementable as either a 3-arity Poseidon directly or as the chained 2-arity form; both cost ~600 R1CS over both sides. Stylistic choice deferred to Phase A's prover library API.
...
Trades the cost of one extra Poseidon input (~100 R1CS total over both sides) for the §3.5 indistinguishability claim being unconditional rather than caveated-by-public-priors. (See §4.7.7 for the full mechanism and remaining threat-model limitations.)
...
10. **Occupancy commitments** (member intermediate + admin intermediate + combined + salt; both sides). ~1200 R1CS for member intermediate (tier-1) + ~600 for admin intermediate + ~600 for the combine + ~600 for absorbing `salt_occ` per side (v0.1.4 — see §4.7.2). The combine binds the two intermediates under `DOMAIN_COMBINED_OCCUPANCY`; the salt-absorb hash binds `salt_occ`; only the final salted output reaches the public-input boundary.
...
| Salt-absorb Poseidon (v0.1.4; 1 hash × 2 sides) | ~600 |
| `c_old`/`c_new` bundled-roots (3 hashes × 2 sides × ~300 each) | ~1800 |
| Existing constraints unchanged | ~3500 |
| **Total tier-1** | **~10230** |

Larger than Democracy's ~6298 — the second tree, the occupancy combine, and v0.1.4's salt absorb add about 62% to the constraint count. Proving time for Oligarchy at tier-1 is roughly 2.9× v1-Anarchy's ~150ms → ~440ms. Still well within budget for an interactive epoch transition.

Tier scaling:
- Tier-0 (member depth 5 / 32 slots): admin overhead unchanged; member overhead drops proportional to slot count. ~9000 constraints.
- Tier-2 (member depth 11 / 2048 slots): member overhead scales 8×. ~22200 constraints.
...
| A2 | Tests for A1 | ~300 | Round-trip member-add, admin-promotion, admin-demotion, member-replace; tombstone-collision regression for both trees; bitmap-mismatch attack rejection per tree; threshold sweep on admin-quorum; target-tree-flip attack (witness `target_tree = 1` while presenting member-delta data — must fail); floor-violation rejection (member→0 or admin→0); §3.5 privacy regression `prove_oligarchy_v2_brute_force_from_priors_rejected` (per §7.1 — v0.1.4 value-level distinguisher test). |
...
- `prove_oligarchy_v2_brute_force_from_priors_rejected` — **the §3.5 privacy regression (v0.1.4 upgrade).** Replaces v0.1.3's wire-shape-only test. Simulates a chain observer with knowledge of §4.8's publicly-fixed initial bitmaps (slot 0 occupied, rest tombstoned, both trees) attempting the brute-force-from-priors attack against the v0.1.4 salted occupancy commitment. The attack runs:
  1. Generate a real Oligarchy update proof (e.g., a member-add taking the group from epoch 0 → 1). Capture `occupancy_commitment_1` from the public-input payload.
  2. The "observer" (the test's adversarial harness) computes `member_occupancy_intermediate_0` and `admin_occupancy_intermediate_0` from the public-priors bitmaps.
  3. The observer enumerates all ~256 single-leaf-delta candidate `member_bitmap_1` plus all ~32 single-leaf-delta candidate `admin_bitmap_1`. For each candidate AND for each of, say, 2^20 randomly-sampled candidate `salt_occ` values, the observer computes the candidate combined commitment and checks against `occupancy_commitment_1`.
  4. **Test asserts no candidate matches** (modulo a vanishingly-small false-positive probability bounded by `(2^20 candidate salts) / 2^252 ≈ 2^-232`). This pins the salt's load-bearing role: the observer cannot recover which tree changed without guessing the 252-bit salt.
  5. The harness additionally asserts that wire-shape indistinguishability is preserved (5 scalars, 192-byte proof, `c_old` identical between two updates from the same prior state) — the v0.1.3 byte-shape property still holds, just superseded by the value-level guarantee.

  **Test fails the build** if (a) the salt is removed from the §4.7.2 formula in a future revision, OR (b) any future change reintroduces a per-tree distinguisher on the wire (the v0.1.3-class regression).
