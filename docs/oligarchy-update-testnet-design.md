...
10. **Occupancy commitments** (member intermediate + admin intermediate + combined + salt; both sides). ~1200 R1CS for member intermediate (tier-1) + ~600 for admin intermediate + ~600 for the combine + ~600 for absorbing `salt_occ` (1 hash × 2 sides; v0.1.4 — see §4.7.2). The combine binds the two intermediates under `DOMAIN_COMBINED_OCCUPANCY`; the salt-absorb hash binds `salt_occ`; only the final salted output reaches the public-input boundary.
...
Larger than Democracy's ~6298 — the second tree, the occupancy combine, and v0.1.4's salt absorb add about 63% to the constraint count. Proving time for Oligarchy at tier-1 is roughly 2.9× v1-Anarchy's ~150ms → ~440ms. Still well within budget for an interactive epoch transition.

Tier scaling (computed as deltas vs tier-1):
- Tier-0 (member depth 5 / 32 slots): admin overhead unchanged; member-side rows scale down (booleans 64 vs 512, binding 128 vs 1024, intermediate Poseidon 600 vs 1200 since the 32-bit bitmap folds to a single scalar). Net ~−1944 from tier-1; **~8300 constraints**.
- Tier-2 (member depth 11 / 2048 slots): member-side booleans (4096) and binding (8192) scale 8× tier-1; member intermediate Poseidon scales with the 9-scalar fold (~5 hashes × 2 sides ≈ 3000 vs tier-1's 1200). Net ~+15000 from tier-1; **~25300 constraints**.
...
**Phase A exit criteria**: `cargo test --features oligarchy-v2-dev-vks circuit::oligarchy_v2` and `cargo test prover::oligarchy_v2_round_trip` both green. R1CS constraint count for tier 1 within 10% of §4.7.3 estimate (~10260). Three dev VKs checked in with fingerprint manifest.
