...
status: v2 — shortened on v1; 13 references; no trusted-setup framing
date: 2026-04-21
---
...
**Aside: why two Poseidon evaluations, not an outer SHA-256.** An earlier version of Onym composed Poseidon inside the circuit with SHA-256 outside it — SHA-256 was chosen because Soroban has a native host function for it. That choice is a constraint-budget landmine: one SHA-256 compression costs roughly 25,000 R1CS constraints, so the small-tier (32-member) circuit was dominated by the outer hash by more than an order of magnitude. Replacing the outer SHA-256 with two Poseidon calls ($\sim 600$ R1CS constraints total) reduces R1CS cost by $8$–$14\times$ across tiers. The lesson — **count R1CS cost before host-function cost when a primitive straddles the circuit boundary** — is the reason we retain this aside even though it is no longer a security postmortem.
...
*Operation-binding.* $\mathcal{A}$ queries an honest prover oracle on $(C_i, \epsilon_i, C'_i)$ and receives valid proofs; then outputs $(\pi^*, C, \epsilon, C^{**})$ with $(C, \epsilon) = (C_i, \epsilon_i)$, $C^{**} \neq C'_i$, such that Verify accepts. Plain knowledge-soundness does **not** discharge this — Groth16 is malleable [6]. Under (A2), the SE extractor produces a witness $w^*$ satisfying $R_{\mathsf{Upd}}(C, \epsilon, C^{**}; w^*)$; since $w^*$ contains $(\mathsf{root}_{\mathsf{new}}, s_{\mathsf{new}})$ opening $C^{**}$, $\mathcal{A}$ knows a preimage of a commitment it claims to have produced without one. The deployed contract incorporates [9]'s public-input-hashing modification; without it, SE of Groth16 is not established. Onym *failed* this game from v1.0.0 through v1.2.x — see P1.
...
Two security postmortems follow. The R1CS-cost episode that earlier drafts called P1 has been demoted to the aside in §5.1; it was a performance lesson, not a security bug, and listing it alongside authorization failures overstated its severity.

### P1 — The theorem that proved the wrong thing
...
### P2 — Transport-layer co-membership leakage
