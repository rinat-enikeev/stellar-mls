...
**Use of generative AI.** The author is a builder, not a cryptographer. Onym was designed, implemented, and deployed before any of §5.3 existed on paper. Turning the working system into writing that a cryptographer would accept as sound required consulting Claude (Anthropic, 2026) extensively — as tutor on the literature and interlocutor on the §5.3 framing choices (SE over plain knowledge-soundness; AGM over GGM; Poseidon-as-RO; Phase-2 setup as load-bearing; R1CS lesson separated from authorization postmortems). The author read the sources Claude surfaced and adopted each framing only after it checked out. This article is a builder's postmortems and design-review lessons, not a protocol proposal; the postmortems are the author's work, and the author is accountable for every technical statement and any remaining errors. The formal scaffolding around the postmortems is accessible because AI made it accessible to a builder.
