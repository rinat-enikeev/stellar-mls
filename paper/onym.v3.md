...
**Use of generative AI.** The author is a builder, not a cryptographer. Onym began as a response to a real metadata-leakage problem in systems the author was already shipping; the registry was designed, implemented, and deployed before any of the formal apparatus in §5.3 existed on paper. To turn a working system into a piece of writing that a cryptographer would accept as sound rather than hand-wavy, the author consulted Claude (Anthropic, 2026) extensively — as a tutor on the literature, an interlocutor for framing choices, and a reader of drafts. The choices that shape §5.3 were arrived at through that dialogue: simulation-extractability rather than plain knowledge-soundness as the property that discharges operation-binding; the algebraic group model in place of the generic group model; out-of-circuit Poseidon modelled as a random oracle rather than claimed as a PRF; the Phase-2 trusted setup treated as a load-bearing deployment condition rather than a footnote; and the R1CS-cost lesson separated from the authorization postmortems. For each, the author read the cited sources Claude surfaced and adopted the framing only after the sources checked out. The author is accountable for every technical statement here and for any remaining errors. What this article is, and is not, matters for how to read the disclosure: it is a set of postmortems and lessons from a builder's perspective — what broke, who caught it, why the preceding reviews did not, and what the review template looks like now — *not* a protocol proposal, a security-proof contribution, or a promotional piece for Onym. The postmortems, the design-review template, and the judgment about what went wrong and why are the author's work; the formal apparatus in §5.3 that surrounds them, which a venue of this kind expects to see next to engineering lessons, is accessible at all because AI made it accessible to a builder.

The uncited inline names in §9 (Railgun, AZTEC, Nocturne, Azeroth) are retained at the genre level: they identify widely-known ZK-pool systems in shape but are not pinned to specific bibliography entries here, and no technical claim in this article rests on any one of them. Any reader re-purposing §9 for a lineage argument should attribute those systems to their authoritative specifications directly rather than inherit our inline naming.
