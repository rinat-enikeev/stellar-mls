...
\begin{abstract}Affordable on-chain Groth16 verification---enabled by Stellar's BLS12-381 host functions---let us gate public-ledger group-membership updates on zero-knowledge proofs with no application-layer operator. This article reports two design postmortems from the alpha deployment: a soundness theorem that bound the wrong predicate, and a transport-layer co-membership leak.
