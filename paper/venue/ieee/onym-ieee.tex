...
\paper{1}
\jmonth{Month}
\jname{IEEE Computer Society Magazine}
...
\begin{abstract}\looseness-1Onym is a Stellar contract that anchors each group's membership state as a 32-byte commitment and gates every state change on a Groth16 zero-knowledge SNARK. The alpha deployment is on Stellar testnet and is scoped to the \emph{unanimous-member} authorization mode, where any one current member may authorize any next state. This article is an experience report: it documents two authorization and privacy postmortems from the deployment, a demoted R1CS-cost lesson, and the design-review template we derived. It is not a cryptographic-proofs contribution, not a production-readiness claim, and not a general messaging substrate. Two deployment facts are load-bearing: the Phase-2 trusted setup currently has a single contributor, so every formal claim is vacuous against that party; and for $n=2$ pair chats, fee-payer unlinkability is trivially defeated by realistic side information. Both belong here rather than buried.
\end{abstract}

\maketitle


\chapteri{M}essaging content is routinely end-to-end encrypted; membership metadata almost never is. Every mainstream architecture we know of has at least one application-layer operator---a server, a federated peer, a Delivery Service\textsuperscript{1}---positioned to observe who is in which group. Signal's Private Group System\textsuperscript{7} shows this can be closed to an honest-but-curious operator using anonymous credentials.
...
\item \emph{a cryptographic-proofs contribution}---the Construction section states what the SNARK binds in the minimum formality the postmortems need; readers who want full simulation-extractability definitions should consult.\textsuperscript{9,11,12}
...
\vspace{4pt}\noindent\textbf{Index Terms:} Zero knowledge proof, cryptographic protocols, blockchains, smart contracts, privacy, authentication.\vadjust{\pagebreak}

\section{CONSTRUCTION}

Let $\Fr$ be the BLS12-381 scalar field and $G_1 \in \Gone$ a generator. Let $H$ denote Poseidon\textsuperscript{5} at the standard BLS12-381 parameter set. For a member set $S = \{\mathsf{pk}_1, \ldots, \mathsf{pk}_n\}$ canonicalized under compressed-G1 serialization, let $\mathsf{root}(S)$ be the Poseidon Merkle root with leaves $H(\mathsf{pk})$. The group commitment is
\begin{equation}
...
Two Groth16 relations,\textsuperscript{2} both proved on BLS12-381:
\begin{itemize}
...
Simulation-extractability (SE)---not plain knowledge-soundness---is the right property here, because Groth16 is malleable:\textsuperscript{6} an adversary can reshape an honest proof into a different accepting proof without a witness. Under SE (in the algebraic group model plus random-oracle heuristic for out-of-circuit Poseidon), any accepting $\Rupd$ proof implies the submitter knows a witness linking the old commitment to the new one via a current member's key, even if the submitter has previously observed simulated proofs. The public-input-hashing modification\textsuperscript{9} (building on prior work\textsuperscript{11}) is what makes SE available for Groth16; the deployed contract uses it.

We do not write the formal games here. The genre of this paper does not require them, the minimal content needed for the postmortems is the paragraph above, and the reader who wants the proofs should go to the cited work\textsuperscript{9,11,12} where they are already written.
...
\item \textbf{MLS integration is a sketch, not a demonstration.} We do not claim to have integrated with a conforming MLS Delivery Service; Mafia\textsuperscript{13} is the directly relevant prior work on the DS-metadata side of the problem and the plausible composition partner.
...
Closest in construction is Semaphore:\textsuperscript{4} both anchor a commitment tree on a public ledger and verify membership with a Groth16 SNARK. Onym differs in that the tree commits to an \emph{evolving group-membership set} rather than a fixed anonymity set, and the authorization predicate is over state \emph{transitions} rather than single-statement membership. Signal PGS\textsuperscript{7} is the gold standard for metadata-private group membership under a trust-the-operator assumption; Onym occupies an adjacent design point (no application-layer operator, public-chain validator set as residual trust). Mafia\textsuperscript{13} addresses the MLS-DS half of the metadata problem and would plausibly compose with Onym in a production stack.
