...
\begin{description}
  \item[\textsc{C1 $\to$ C7} (in-place SNARK swap):] retain the
    ...
    \textbf{(b)}: Plonky3 itself is mature enough for production use,
    but Stellar Protocol 22's host-function set (BLS12-381 pairings
    only~\cite{stellar-cap-0058}) does not accelerate FRI
    verification. Pure-WASM FRI verification is one to two orders of
    magnitude more expensive than C1's pairing-precompile path. A
    Stellar protocol upgrade adding Goldilocks-arithmetic and
    Merkle-friendly hash host functions would unblock this path; we
    discuss the technical shape of such a CAP in
    \S\ref{sec:open}.
...
\begin{description}
  ...
  \item[\textsc{C2 $\to$ C1} (curve and SNARK retained, anchor and
    host changed):] re-deploy on Stellar. Class \textbf{(a)}; the
    transformation is purely operational. Does not improve PQ
    posture; included for completeness as a non-PQ migration that
    nonetheless changes the trust posture.
\end{description}
...
\begin{description}
  \item[\textsc{Stellar protocol upgrade adding FRI host functions}:]
    Class \textbf{(b)}. The technical shape of such an upgrade is a
    Goldilocks-arithmetic precompile, a Poseidon2-or-Rescue host
    function, and ideally an NTT primitive; CAP-style proposals
    along these lines do not exist at the time of writing. We
    estimate the engineering scale at one to two
    chain-protocol-upgrade cycles, plus the validator-side activation
    process; we do not estimate the political feasibility, which is
    the load-bearing variable in practice.
\end{description}
