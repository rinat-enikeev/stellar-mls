...
///
/// **Non-standard round constants (M-1):** Round constants are derived from a
/// custom SHA-256 seed (`"SEP-XXXX-Poseidon-BLS12-381-w3-f8-p56-a5-round-constants"`)
/// rather than the canonical generation method from the Poseidon paper
/// (Grassi et al., Section 6 / `generate_params_poseidon.sage`).
///
/// This means proofs generated by this library are **not compatible** with
/// other Poseidon implementations that use the reference constants. This is
/// intentional: SEP-XXXX uses a self-contained parameter set to avoid external
/// dependencies and ensure deterministic reproducibility across platforms.
///
/// The MDS matrix uses the standard Cauchy construction and is not affected.
///
/// If interoperability with external Poseidon implementations is required in
/// the future, replace `generate_round_constants()` with constants from the
/// reference generation script and re-run key generation (all existing proofs
/// and commitments would need migration).
...
/// Generate deterministic round constants from a domain-separated SHA-256 seed.
///
/// **Non-standard:** This uses a custom seed string rather than the Poseidon
/// paper's reference generation script. See `poseidon_config()` doc comment
...
fn generate_round_constants<F: PrimeField>(count: usize) -> Vec<F> {
    ...
    // The seed encodes: width=3, full_rounds=8, partial_rounds=56, alpha=5.
